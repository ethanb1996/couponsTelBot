...
func main() {
  ...
  queries := []struct{ label, sql string }{
    ...
    {"discounted_imported", `select count(*) from listings where external_import_key <> '' and coupon_value_amount > sale_price_amount`},
    {"active_discounted_imported", `select count(*) from listings where external_import_key <> '' and status = 'active' and coupon_value_amount > sale_price_amount`},
    {"active_discounted_with_inventory", `
      select count(*)
      from listings l
      where l.status = 'active'
        and l.coupon_value_amount > l.sale_price_amount
        and exists (
  ...
  for _, q := range queries {
    var count int64
    if err := db.Pool.QueryRow(ctx, q.sql).Scan(&count); err != nil { panic(err) }
    fmt.Printf("%s=%d\n", q.label, count)
  }

  rows, err := db.Pool.Query(ctx, `
    select l.id, l.status, l.merchant_name, l.title, l.coupon_value_amount, l.sale_price_amount,
    ...
    where l.coupon_value_amount > l.sale_price_amount
    group by l.id
  ...
  defer rows.Close()
  for rows.Next() {
    var id, value, sale, available int64
    var status, merchant, title string
    if err := rows.Scan(&id, &status, &merchant, &title, &value, &sale, &available); err != nil { panic(err) }
    fmt.Printf("id=%d status=%s available=%d value=%d sale=%d merchant=%s title=%s\n", id, status, available, value, sale, merchant, title)
  }
}
